...
--- helpers
local function update_vocab(words, vc)
    for i, word in pairs(words) do
        ...
        if #word > 0 then
            ...
            -- in or out vocab?
            if not vc[word] then -- insert new word
                vc[word] = 1
            else -- in vocab
                vc[word] = vc[word] + 1
            end
...
local function update_vocab_old(words, v, vc)
...
this.main = function (opt)
    ...
    for i, line in ipairs(lines) do
        ...
        update_vocab(words, vocabFreq)
        xlua.progress(i, #lines)
    end
    print('#vocab = ' .. tablex.size(vocabFreq))
    time = torch.toc(time)
    print('elapsed time for vocab extraction = ' .. (time/60) .. ' min')
