...
local utmisc = require'util.misc' -- print, etc...
local ev = require'eval' -- eval_dataset
...
this.main = function(opt)
    ...
    print('[data]')
    local tmpRet = dofile(opt.dataPath).main(opt)
    local loaderTr, loaderVal, loaderTe = tmpRet.tr, tmpRet.val, tmpRet.te
    ...
    for i = begIt + 1, maxIt do
        ...
        if i % opt.evalFreq == 0 then
            lossesVal[i], errVal[i] = ev.eval_dataset(loaderVal, md, cri, opt.numClasses)
